...
Citizen.CreateThread(function()
	...
	RMenu.Add('personal', 'admin', RageUI.CreateSubMenu(RMenu.Get('rageui', 'personal'), _U('admin_title_menu')), function()
...
-- Weapon Menu --
RegisterNetEvent('Zouindo-PersonalMenu:Weapon_addAmmoToPedC')
AddEventHandler('Zouindo-PersonalMenu:Weapon_addAmmoToPedC', function(value, quantity)
...
-- Admin Menu --
RegisterNetEvent('Zouindo-PersonalMenu:Admin_BringC')
AddEventHandler('Zouindo-PersonalMenu:Admin_BringC', function(plyCoords)
...
function RenderPersonalMenu()
	RageUI.DrawContent({header = true, instructionalButton = true}, function()
		...
		RageUI.Button(_U('wallet_money_separator'), nil, {}, true, function(Hovered, Active, Selected)
			if (Selected) then
				local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()

				if closestDistance ~= -1 and closestDistance <= 3.0 then
					TriggerServerEvent('rien', GetPlayerServerId(PlayerId()), GetPlayerServerId(closestPlayer), 'rien')
				else
				end
			end
		end)
		...
		elseif type == 'loadout' then
			if HasPedGotWeapon(plyPed, PersonalMenu.ItemSelected.hash, false) then
				...
				RageUI.Button(_U('loadout_givemun_button'), "", {RightBadge = RageUI.BadgeStyle.Ammo}, true, function(Hovered, Active, Selected)
					if (Selected) then
						local post, quantity = CheckQuantity(KeyboardInput('Zouindo_BOX_AMMO_AMOUNT', _U('dialogbox_amount_ammo'), '', 8))
								...
								if not IsPedSittingInAnyVehicle(closestPed) then
									...
									if ammo > 0 then
										if quantity <= ammo and quantity >= 0 then
											...
											TriggerServerEvent('Zouindo-PersonalMenu:Weapon_addAmmoToPedS', GetPlayerServerId(closestPlayer), PersonalMenu.ItemSelected.name, quantity)
...
function RenderWalletMenu()
	RageUI.DrawContent({header = true, instructionalButton = true}, function()
		...
		RageUI.List(_U('wallet_money_button', ESX.Math.GroupDigits(ESX.PlayerData.money)), PersonalMenu.WalletList, PersonalMenu.WalletIndex['money'] or 1, nil, {}, true, function(Hovered, Active, Selected, Index)
			if (Selected) then
				if Index == 1 then
					local post, quantity = CheckQuantity(KeyboardInput('Zouindo_BOX_AMOUNT', _U('dialogbox_amount'), '', 8))
			...
			RageUI.Button(PersonalMenu.BillData[i].label, nil, {RightLabel = '$' .. ESX.Math.GroupDigits(PersonalMenu.BillData[i].amount)}, true, function(Hovered, Active, Selected)
				if (Selected) then
					ESX.TriggerServerCallback('esx_billing:payBill', function()
						ESX.TriggerServerCallback('Zouindo-PersonalMenu:Bill_getBills', function(bills)
			...
			if (Selected) then
				if ESX.PlayerData.job.grade_name == 'boss' then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						TriggerServerEvent('Zouindo-PersonalMenu:Boss_recruterplayer', GetPlayerServerId(closestPlayer), ESX.PlayerData.job.name, 0)
					end
				else
			...
			if (Selected) then
				if ESX.PlayerData.job.grade_name == 'boss' then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						TriggerServerEvent('Zouindo-PersonalMenu:Boss_virerplayer', GetPlayerServerId(closestPlayer))
					end
				else
					ESX.ShowNotification(_U('missing_rights'))
				end
			...
			if (Selected) then
				if ESX.PlayerData.job.grade_name == 'boss' then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						TriggerServerEvent('Zouindo-PersonalMenu:Boss_promouvoirplayer', GetPlayerServerId(closestPlayer))
					end
				else
					ESX.ShowNotification(_U('missing_rights'))
				end
			...
			if (Selected) then
				if ESX.PlayerData.job.grade_name == 'boss' then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						TriggerServerEvent('Zouindo-PersonalMenu:Boss_destituerplayer', GetPlayerServerId(closestPlayer))
					end
				else
					ESX.ShowNotification(_U('missing_rights'))
				end
			...
			if (Selected) then
				if ESX.PlayerData.job2.grade_name == 'boss' then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						TriggerServerEvent('Zouindo-PersonalMenu:Boss_recruterplayer2', GetPlayerServerId(closestPlayer), ESX.PlayerData.job2.name, 0)
					end
				else
					ESX.ShowNotification(_U('missing_rights'))
				end
			...
			if (Selected) then
				if ESX.PlayerData.job2.grade_name == 'boss' then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						TriggerServerEvent('Zouindo-PersonalMenu:Boss_virerplayer2', GetPlayerServerId(closestPlayer))
					end
				else
					ESX.ShowNotification(_U('missing_rights'))
				end
			...
			if (Selected) then
				if ESX.PlayerData.job2.grade_name == 'boss' then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						TriggerServerEvent('Zouindo-PersonalMenu:Boss_promouvoirplayer2', GetPlayerServerId(closestPlayer))
					end
				else
					ESX.ShowNotification(_U('missing_rights'))
				end
			...
			if (Selected) then
				if ESX.PlayerData.job2.grade_name == 'boss' then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						TriggerServerEvent('Zouindo-PersonalMenu:Boss_destituerplayer2', GetPlayerServerId(closestPlayer))
					end
				else
					ESX.ShowNotification(_U('missing_rights'))
				end
	...
	while true do
		...
		if IsControlJustReleased(0, Config.Controls.OpenMenu.keyboard) and not Player.isDead then
			if not RageUI.Visible() then
				ESX.TriggerServerCallback('Zouindo-PersonalMenu:Admin_getUsergroup', function(plyGroup)
