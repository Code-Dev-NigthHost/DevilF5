...
-- GENERAL --
Config.MenuTitle = 'F5 by NeyRox' -- change it to you're server name
...
Config.GPS = {
	...
	{label = 'Police', coords = vector2(425.13, -979.55)},
	{label = 'Parking Central', coords = vector2(-449.67, -340.83)},
...
Config.Admin = {
	{
		...
		groups = {'helpeur', 'owner', 'superadmin', 'admin', 'mod'},
		command = function()
			local plyId = KeyboardInput('Zouindo_BOX_ID', _U('dialogbox_playerid'), '', 8)

			if plyId ~= nil then
				plyId = tonumber(plyId)
				
				if type(plyId) == 'number' then
					TriggerServerEvent('Zouindo-PersonalMenu:Admin_BringS', GetPlayerServerId(PlayerId()), plyId)
				end
			end
	...
	},
	{
		...
		command = function()
			local pos = KeyboardInput('Zouindo_BOX_XYZ', _U('dialogbox_xyz'), '', 50)
	...
	},
	{
		...
		groups = {'_dev', 'owner', 'superadmin'},
		command = function()
			local modelName = KeyboardInput('Zouindo_BOX_VEHICLE_NAME', _U('dialogbox_vehiclespawner'), '', 50)
	...
	},
	{
		...
		groups = {'_dev', 'owner', 'superadmin'},
		command = function()
			local amount = KeyboardInput('Zouindo_BOX_AMOUNT', _U('dialogbox_amount'), '', 8)

			if amount ~= nil then
				amount = tonumber(amount)

				if type(amount) == 'number' then
					TriggerServerEvent('Zouindo-PersonalMenu:Admin_giveCash', amount)
				end
			end
	...
	},
	{
		...
		command = function()
			...
			if amount ~= nil then
				...
				if type(amount) == 'number' then
					TriggerServerEvent('Zouindo-PersonalMenu:Admin_giveBank', amount)
				end
			end
	...
	},
	{
		...
		command = function()
			...
			if amount ~= nil then
				...
				if type(amount) == 'number' then
					TriggerServerEvent('Zouindo-PersonalMenu:Admin_giveDirtyMoney', amount)
				end
			end
	...
	},
	{
		...
		groups = {'mod', 'helpeur', 'superadmin', 'admin'},
		command = function()
			local plyId = KeyboardInput('Zouindo_BOX_ID', _U('dialogbox_playerid'), '', 8)
